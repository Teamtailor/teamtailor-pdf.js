...
if (typeof PDFJSDev === "undefined" || PDFJSDev.test("GENERIC")) {
  ...
  const TEAMTAILOR_VIEWER_ORIGIN_SUFFIXES = [".teamtailor.localhost"];
  ...
  var validateFileURL = function (file) {
    ...
    if (
      ...
      TEAMTAILOR_VIEWER_ORIGIN_SUFFIXES.some(suffix =>
        viewerOrigin.endsWith(suffix)
      )
    ) {
    ...
    if (
      fileOrigin &&
      (TEAMTAILOR_S3_BUCKET_REG_EX.test(fileOrigin) ||
        TEAMTAILOR_CDN_REG_EX.test(fileOrigin))
    ) {
      return;
    }
